// fhtml.js — Functional HTML builder + static file generator for Bun
...
// All style() calls accumulate here; page() flushes them into a single <style>
let _styles = [];
...
function style(propsOrCss, ...rest) {
    ...
    const children = isProps(propsOrCss) ? rest : [propsOrCss, ...rest];

    const { href, ...attrs } = props;

    // External stylesheet — emit immediately as <link>, don't accumulate
    if (href) {
        return new RawHtml(`<link rel="stylesheet"${buildAttrs({ href, ...attrs })} />`);
    }
...
function page(rootNode) {
    ...
    if (stylesByAttrs.size > 0) {
        const styleTags = [...stylesByAttrs.values()]
            ...
            .join("\n");
        withStyles = withStyles.replace("</head>", `${styleTags}\n</head>`);
    }
...
export { tag, raw, render, page, generate, script, style, createHead };
